...
const MobileTabButton = ({ active, icon: Icon, label, onClick }) => {
  return (
    ...
    >
      <Icon size={18} />
      <span>{label}</span>
      {active && <span className="mt-1 h-[3px] w-8 rounded-full bg-white" />}
    </button>
  );
};
  ...
  return (
    ...
    >
      <div className="flex min-h-screen flex-col lg:hidden">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-[70] border-t border-slate-700 bg-[#0f171c] pb-[env(safe-area-inset-bottom)]">
          <div className="grid grid-cols-5">
            ...
            <MobileTabButton
              ...
              icon={LayoutGrid}
              label="Instruments"
              onClick={() => setMobileView("instruments")}
            />
            <MobileTabButton
              active={mobileView === "positions"}
              icon={Briefcase}
        ...
        {mobileAccountsOpen && (
          <div className="fixed inset-0 z-[120] bg-black/60 backdrop-blur-[2px]">
            <div className="absolute bottom-0 left-0 right-0 rounded-t-[28px] border-t border-slate-700 bg-[#111c22] shadow-2xl">
              <div className="flex justify-center py-3">
                <div className="h-1.5 w-14 rounded-full bg-slate-600" />
              </div>
