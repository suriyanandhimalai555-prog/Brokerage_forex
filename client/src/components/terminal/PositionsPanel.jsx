...
const PositionsPanel = ({
  ...
  const handleSaveProtection = async () => {
    ...
    try {
      ...
      const tp =
        tpValue === "" ? null : Number(tpValue);

      const sl =
        slValue === "" ? null : Number(slValue);
                ...
                rows.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="border-t border-slate-700/60">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center gap-2 ${String(order.type).toLowerCase().startsWith("buy")
                              ? "text-sky-400"
                              : "text-rose-400"
                            }`}
                        >
                          <span
                            className={`h-2 w-2 rounded-full ${String(order.type).toLowerCase().startsWith("buy")
                                ? "bg-sky-400"
                                : "bg-rose-400"
                              }`}
                          />
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        {Number(livePrice || 0).toFixed(
                          String(order.symbol || "").includes("JPY")
                            ? 3
                            : String(order.symbol || "").includes("EUR")
                              ? 5
                              : String(order.symbol || "").includes("GBP")
                                ? 5
                                : String(order.symbol || "").includes("BTC")
                                  ? 2
                                  : 3
                        )}
                      </td>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-[#111c22] p-5 text-slate-100 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold">Edit Take Profit / Stop Loss</h3>
                <p className="text-sm text-slate-400">
                  {selectedOrder.display_symbol || cleanSymbol(selectedOrder.symbol)}
                </p>
              </div>
